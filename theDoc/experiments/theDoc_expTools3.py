...
from keras.initializers import RandomNormal, RandomUniform, Ones, Zeros, lecun_uniform
...
class kfold_cv():
    ...
    def run_kfold_cv(self,datamaster,n_splits=6):
        ...
        def fold_actions(X_train, X_test, y_train, y_test):
            ...
            X_train_ho, X_train_ao, X_test_ho, X_test_ao = np.array(X_train_ho.reset_index(drop=True)), np.array(X_train_ao.reset_index(drop=True)), np.array(X_test_ho.reset_index(drop=True)), np.array(X_test_ao.reset_index(drop=True))
            ...
            for ensemble_mod in range(num_ensemble_models):
                ...
                # Def custom function instead of tf function:
                def tf_batchcorrelate(inp, name=None):

                    with ops.name_scope(name, "batchCorrelate", [inp]) as name:
                        bCorr = py_func(batchcorrelate,
                                        [inp],
                                        [tf.float32],
                                        name=name,
                                        grad=_MyBatchCorrelateGrad)  # <-- here's the call to the gradient
                        bCorr = bCorr[0]
                        bCorr.set_shape((None,inp[0].shape[-1]-1))
                        return bCorr

                def batchcorrelate(i):
                    out = []
                    input_nodes = int(i.shape[-1]/2)
                ...
                # Actual gradient:
                def _MyBatchCorrelateGrad(op, grad):
                    samples = op.inputs[0].get_shape().as_list()[0]
                    if samples is None:
                        samples = batch_size
                    tot_input_nodes = op.inputs[0].get_shape().as_list()[-1]
                    input_nodes = int(tot_input_nodes/2)
                    output_nodes = tot_input_nodes - 1

                    new_grad = []

                    for sample in range(samples):

                        for inputspart in range(tot_input_nodes):

                            if inputspart == 0:

                                z = tf.zeros(([1,input_nodes-1]),dtype=tf.float32)
                                c = tf.reverse(tf.reshape(op.inputs[0][sample][input_nodes:],[1,-1]),[1])
                                grad_matrix = tf.concat([c,z],1)

                            elif inputspart < input_nodes:

                                z_b = tf.zeros(([1,inputspart]),dtype=tf.float32)
                                z_e = tf.zeros(([1,input_nodes-inputspart-1]),dtype=tf.float32)
                                c = tf.reverse(tf.reshape(op.inputs[0][sample][input_nodes:],[1,input_nodes]),[1])
                                grad_line = tf.concat([z_b,c,z_e],1)
                                grad_matrix = tf.concat([grad_matrix,grad_line],0)

                            elif inputspart == input_nodes:

                                z = tf.zeros(([1,input_nodes-1]),dtype=tf.float32)
                                c = tf.reshape(op.inputs[0][sample][:input_nodes],[1,-1])
                                grad_line = tf.concat([c,z],1)
                                grad_matrix = tf.concat([grad_matrix,grad_line],0)

                            elif inputspart > input_nodes:

                                z_b = tf.zeros(([1,inputspart-input_nodes]),dtype=tf.float32)
                                z_e = tf.zeros(([1,input_nodes-inputspart+input_nodes-1]),dtype=tf.float32)
                                c = tf.reshape(op.inputs[0][sample][:input_nodes],[1,-1])
                                grad_line = tf.concat([z_b,c,z_e],1)
                                grad_matrix = tf.concat([grad_matrix,grad_line],0)

                        new_grad.append(grad_matrix)

                    jactrans = tf.stack(new_grad,0)
                    reshaped_grad = tf.reshape(grad,[-1,output_nodes,1])

                    fin_grad = tf.matmul(jactrans,reshaped_grad)
                    fin_grad = tf.reshape(fin_grad,[-1,tot_input_nodes])
                ...
                # Def custom function instead of tf function:
                def tf_batchconvolve(inp, name=None):

                    with ops.name_scope(name, "batchConvolve", [inp]) as name:
                        bConv = py_func(batchconvolve,
                                        [inp],
                                        [tf.float32],
                                        name=name,
                                        grad=_MyBatchConvolveGrad)  # <-- here's the call to the gradient
                        bConv = bConv[0]
                        bConv.set_shape((None,inp[0].shape[-1]-1))
                        return bConv
                ...
                # Actual gradient:
                def _MyBatchConvolveGrad(op, grad):
                    samples = op.inputs[0].get_shape().as_list()[0]
                    if samples is None:
                        samples = batch_size
                    tot_input_nodes = op.inputs[0][0].get_shape().as_list()[-1]
                    input_nodes = int(tot_input_nodes/2)
                    output_nodes = tot_input_nodes - 1
                    ...
                    for sample in range(samples):
                        ...
                        for inputspart in range(tot_input_nodes):
                            ...
                            if inputspart == 0:
                                ...
                                z_b = tf.zeros(([1,inputspart]),dtype=tf.float32)
                                z_e = tf.zeros(([1,input_nodes-inputspart-1]),dtype=tf.float32)
                                c = tf.reshape(op.inputs[0][sample][input_nodes:],[1,input_nodes])
                                grad_line = tf.concat([z_b,c,z_e],1)
                                grad_matrix = tf.concat([grad_matrix,grad_line],0)
                ...
                output_dummyscores = mergeconcatenate([output_dummyhomescore,output_dummyawayscore],axis=1)

                batchcorrelate_layer = core.Lambda(tf_batchcorrelate, name='dummydif')
                output_dummyruns = batchcorrelate_layer(output_dummyscores)

                batchconvolve_layer = core.Lambda(tf_batchconvolve, name='dummytots')
                output_dummytots = batchconvolve_layer(output_dummyscores)
                ...
                model.compile(loss='categorical_crossentropy',
                              #optimizer=Adam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.001)
                ...
                probas = model.predict({"ho_input":X_test_ho,"ao_input":X_test_ao}, batch_size=256, verbose=0)
